...
async function uploadToCloudinary(file: File): Promise<string> {
  ...
  // Fallback: base64 data URL (works without Cloudinary, but URL won't be stable)
  ...
  form.append('folder',         'blvcrea/inspirations');
  ...
  if (formStatus === 'success') {
    return (
      <div className="text-center py-14 px-6">
        ...
        <p className="font-sans text-blush-500 max-w-md mx-auto leading-relaxed mb-8">
          Merci pour votre message. Nous vous répondrons dans les <strong>48 heures</strong> pour discuter de votre beau projet sur-mesure.
        </p>
    ...
    <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-6">
      ...
      <div>
          ...
          {COLOR_OPTIONS.map(c => (
            ...
            >
              {selectedColor === c.value && (
                <span className={`absolute inset-0 flex items-center justify-center text-xs ${c.dark ? 'text-white' : 'text-gray-700'}`}>
                  ✓
                </span>
              )}
            </button>
        ...
        {selectedColor && (
          <p className="mt-2 font-sans text-xs text-blush-500">
            Sélectionnée : <strong>{selectedColor}</strong>
          </p>
        )}
      ...
      <div>
        ...
        <textarea
          ...
          rows={5}
          placeholder="Dimensions souhaitées, style, pièce de la maison (chambre, salon…), occasion (cadeau, décoration…), matières préférées…"
      ...
      <div>
        ...
        <p className="font-sans text-xs text-blush-400 mb-3">
          Une photo Pinterest, Instagram ou une idée que vous avez repérée. JPG / PNG / WebP, max 5 Mo.
        </p>
        ...
        {imageUrl ? (
          ...
          <div className="flex items-start gap-4 p-4 bg-petal rounded-2xl border border-rose-100">
            <div className="relative w-20 h-20 flex-shrink-0 rounded-xl overflow-hidden border border-rose-200">
              ...
              <img
                src={imageUrl.startsWith('data:') ? imageUrl : `${imageUrl}?w=160`}
                alt="Votre image d'inspiration"
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="font-sans text-xs text-blush-400 mt-1 truncate">
                {imageUrl.startsWith('data:') ? 'Image locale (aperçu)' : imageUrl}
              </p>
              <button
                type="button"
          ...
          <button
            ...
            disabled={uploading}
            className="w-full flex flex-col items-center justify-center gap-3 px-6 py-8 border-2 border-dashed border-rose-200 rounded-2xl text-blush-400 hover:border-blush-300 hover:bg-petal/60 hover:text-blush-500 transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {uploading ? (
              <>
                <Loader2 size={28} className="animate-spin text-blush-400" />
                <span className="font-sans text-sm">Chargement de l&apos;image…</span>
              </>
            ) : (
              <>
                <div className="w-12 h-12 rounded-2xl bg-petal flex items-center justify-center">
                  <ImageIcon size={22} className="text-blush-400" />
                </div>
        ...
        {uploadErr && (
          <p className="mt-2 flex items-center gap-1.5 font-sans text-xs text-red-500">
      ...
      <p className="font-sans text-xs text-center text-blush-400">
        Réponse sous 48h · Devis gratuit · Vos données restent confidentielles
      </p>
    </form>
  );
}
